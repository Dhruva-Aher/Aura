...
type ScenarioResult = {
  ...
  p95ProcessingSec: number;
  peakBacklog: number;
  drainRate: number;
  retryRate: number;
  failureRate: number;
  duplicateCompletedJobs: number;
  jobLoss: number;
  recovered: boolean;
};
...
function pickQueue(i: number): 'high' | 'default' | 'low' {
  if (i % 10 === 0) return 'high';
  if (i % 7 === 0) return 'low';
  return 'default';
}

async function enqueueJobs(runId: string, total: number, injectionConcurrency: number) {
  let submitted = 0;
  let rejected = 0;
  let index = 0;
...
async function collectBacklogPeak(runId: string, stopSignal: { stop: boolean }) {
  let peak = 0;
...
async function waitForDrain(runId: string, expected: number, timeoutMs: number) {
  const start = Date.now();
  while (Date.now() - start < timeoutMs) {
...
async function runScenario(jobs: number, workerProcesses: number, workerConcurrency: number): Promise<ScenarioResult> {
  ...
  const backlogPeakPromise = collectBacklogPeak(runId, backlogStop);
  ...
  const duplicateRows = await prisma.jobEvent.groupBy({
    by: ['jobId'],
  ...
  return {
    ...
    duplicateCompletedJobs: duplicateRows.length,
    jobLoss,
